...
//   const completedIds: string[] = JSON.parse(localStorage.getItem("completedLessons") || "[]");
//   const completedLessons = completedIds.map((id) => {
//     const lesson = lessonList.find((l) => l.id === id);
//     return lesson || { id, name: "Unknown", image: "", description: "", level: "Beginner" };
//   });
...
//     let progress = progressRaw ? JSON.parse(progressRaw) : {};
...
//         <>
//           {/* Left: Lessons */}
//           <div style={{ overflowY: "auto", paddingRight: 16 }}>
//             <SectionTitle text="Your Courses" color={textColor} />
//             <LessonList lessons={startedLessons} dark={dark} completed={false} />

//             <SectionTitle text="Completed Courses" color={textColor} style={{ marginTop: 32 }} />
//             <LessonList lessons={completedLessons} dark={dark} completed={true} />
//           </div>

//           {/* Right: Analytics */}
//           <div style={{ overflowY: "auto", paddingLeft: 16, display: "flex", flexDirection: "column", gap: 24 }}>
//             <QuizDashboardSummary dark={dark} />

//             <div style={{ display: "flex", flexWrap: "wrap", gap: 18, justifyContent: "center" }}>
//               {/* Lesson Completion Pie */}
//               <Card dark={dark} width={300}>
//                 <div style={{ fontWeight: 900, fontSize: 18, marginBottom: 10, color: textColor }}>Lesson Completion</div>
//                 <ResponsiveContainer width="100%" height={180}>
//                   <PieChart>
//                     <Pie data={lessonPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} label>
//                       {lessonPieData.map((entry, idx) => (
//                         <Cell key={`cell-${idx}`} fill={lessonPieColors[idx % lessonPieColors.length]} />
//                       ))}
//                     </Pie>
//                     <Tooltip />
//                     <Legend />
...
//                   <div style={{ fontWeight: 900, fontSize: 18, marginBottom: 10, color: textColor }}>Quiz Scores</div>
//                   <ResponsiveContainer width="100%" height={180}>
//                     <BarChart data={quizBarData} margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
//                       <CartesianGrid strokeDasharray="3 3" />
//                       <XAxis dataKey="name" stroke={dark ? '#f8f5f0' : '#23283a'} />
...
//   <div style={{ fontWeight: 900, fontSize: 22, marginBottom: 18, color, letterSpacing: 0.5, fontFamily: "'Lato', Arial, sans-serif", textAlign: "left", ...style }}>
//     {text}
//   </div>
// );
...
// const LessonList = ({ lessons, dark, completed }: any) => (
//   <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
//     {lessons.map((lesson: any) => {
//       const topics = lessonTopics[lesson.id] || [];
//       const allIds: string[] = [];
//       topics.forEach((t: any) => t.subtopics?.length ? t.subtopics.forEach((s: any) => allIds.push(`${t.id}__${s.id}`)) : allIds.push(t.id));
...
//       const percent = completed ? 100 : (allIds.length > 0 ? Math.round((readItems.length / allIds.length) * 100) : 0);
...
//             display: "flex",
//             alignItems: "center",
//             background: dark ? "linear-gradient(135deg, #23283a 60%, #2d3650 100%)" : "linear-gradient(135deg, #f5f8ff 60%, #e3eaff 100%)",
//             borderRadius: 22,
//             boxShadow: dark ? "0 8px 32px #000a, 0 1.5px 0 #fff2" : "0 8px 32px #bcd6ff44, 0 1.5px 0 #0001",
//             padding: 10,
//             minHeight: "5vh",
//           }}
//         >
...
//             src={lesson.image}
//             alt={lesson.name}
//             style={{ width: "8%", height: "auto", objectFit: "cover", borderRadius: 10, marginRight: 24, boxShadow: dark ? "0 1px 6px #0004" : "0 1px 6px #0001" }}
//           />
//           <div style={{ flex: 1, textAlign: "left" }}>
//             <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4, color: dark ? "#f8f5f0" : "#23283a", fontFamily: "'Lato', Arial, sans-serif" }}>{lesson.name}</div>
//             <div style={{
...
//               background: lesson.level === "Beginner" ? "linear-gradient(90deg,#4caf50,#81c784)" :
//                           lesson.level === "Intermediate" ? "linear-gradient(90deg,#ff9800,#ffc107)" :
